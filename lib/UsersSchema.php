...
{
    private const ADMIN_USERNAME = 'admin@fullvendor.com';
    ...
    public static function ensure(): void
    {
        ...
        try {
            ...
            self::ensureCatalogProductsTable($pdo);
            self::ensureCatalogSyncStateTable($pdo);
            self::ensureOrdersTable($pdo);
            self::ensureOrderDetailsTable($pdo);
            self::ensureCartOrdersTable($pdo);
            self::ensureCartDetailsTable($pdo);
            self::ensurePortalCartHeaderTable($pdo);
            self::ensurePortalCartItemTable($pdo);
            self::ensureCustomerGroupsTable($pdo);
            self::ensureSellerCatalogUserStateTable($pdo);
            self::ensureCustomerCatalogUserStateTable($pdo);
            self::ensureMessagingTables($pdo);
            self::seedAdminUser($pdo);
        } catch (Throwable $e) {
            error_log('[UsersSchema] ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
            if (class_exists('AppLog', false)) {
                AppLog::app(
                    '[UsersSchema] ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine(),
                    'ERROR'
                );
            }
        }
    ...
    private static function ensureCustomersTable(\PDO $pdo): void
    {
        $exists = $pdo->query("SHOW TABLES LIKE 'customers'");
        if ($exists === false || $exists->rowCount() === 0) {
            $pdo->exec(self::customersTableSql());
        } else {
            self::migrateCustomersColumns($pdo);
        }
    }

    private static function migrateCustomersColumns(\PDO $pdo): void
    {
        ...
        $parts = [];
        if (!isset($have['customeridfullvendor'])) {
            $parts[] = 'ADD COLUMN `customeridfullvendor` BIGINT NULL DEFAULT NULL AFTER `customer_id`';
        }
        if (!isset($have['term_name'])) {
            $parts[] = 'ADD COLUMN `term_name` VARCHAR(255) DEFAULT \' \' AFTER `term_id`';
        }
        if (!isset($have['group_name'])) {
            $parts[] = 'ADD COLUMN `group_name` VARCHAR(255) DEFAULT \' \' AFTER `group_id`';
        }
        if (!isset($have['percentage_on_price'])) {
            $parts[] = 'ADD COLUMN `percentage_on_price` FLOAT DEFAULT 0 AFTER `group_name`';
        }
        if (!isset($have['percent_price_amount'])) {
            $parts[] = 'ADD COLUMN `percent_price_amount` FLOAT DEFAULT 0 AFTER `percentage_on_price`';
        }

        if ($parts !== []) {
            $pdo->exec('ALTER TABLE `customers` ' . implode(', ', $parts));
        }

        $chk = $pdo->prepare(
            'SELECT 1 FROM information_schema.STATISTICS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND INDEX_NAME = ?'
        );
        $chk->execute([$db, 'customers', 'uq_customeridfullvendor']);
        if ($chk->fetch() === false) {
            try {
                $pdo->exec(
                    'ALTER TABLE `customers` ADD UNIQUE KEY `uq_customeridfullvendor` (`customeridfullvendor`)'
                );
            } catch (Throwable) {
            }
        }
    ...
    {
        $pdo->exec(self::catalogCategoriesTableSql());
    }

    private static function ensureCatalogProductsTable(\PDO $pdo): void
    {
        $pdo->exec(self::catalogProductsTableSql());
    }

    private static function ensureCatalogSyncStateTable(\PDO $pdo): void
    {
        $pdo->exec(self::catalogSyncStateTableSql());
        try {
            $pdo->exec("INSERT IGNORE INTO `catalog_sync_state` (`id`, `status`) VALUES (1, 'idle')");
        } catch (Throwable) {
        }
    }

    private static function catalogSyncStateTableSql(): string
    {
        return <<<'SQL'
...
SQL;
    ...
    private static function ensureOrdersTable(\PDO $pdo): void
    {
        $pdo->exec(self::ordersTableSql());
    }

    private static function ensureOrderDetailsTable(\PDO $pdo): void
    {
        $pdo->exec(self::orderDetailsTableSql());
    }

    private static function ensureCartOrdersTable(\PDO $pdo): void
    {
        $pdo->exec(self::cartOrdersTableSql());
    }

    private static function ensureCartDetailsTable(\PDO $pdo): void
    {
        $pdo->exec(self::cartDetailsTableSql());
    }

    private static function ensurePortalCartHeaderTable(\PDO $pdo): void
    {
        $pdo->exec(self::portalCartHeaderTableSql());
    }

    private static function ensurePortalCartItemTable(\PDO $pdo): void
    {
        $pdo->exec(self::portalCartItemTableSql());
        self::migratePortalCartItemColumns($pdo);
    }

    private static function migratePortalCartItemColumns(\PDO $pdo): void
    {
        $db = trim((string) config('DB_NAME', ''));
        if ($db === '') {
            return;
        ...
        $st->execute([$db, 'portal_cart_item']);
        $have = [];
        foreach ($st->fetchAll(PDO::FETCH_COLUMN) as $col) {
            $have[strtolower((string) $col)] = true;
        }
        if (!isset($have['line_note'])) {
            try {
                $pdo->exec(
                    'ALTER TABLE `portal_cart_item` ADD COLUMN `line_note` TEXT NULL DEFAULT NULL AFTER `moq`'
                );
            } catch (Throwable) {
            }
        }
    ...
    private static function portalCartHeaderTableSql(): string
    {
        return <<<'SQL'
...
SQL;
    ...
    private static function portalCartItemTableSql(): string
    {
        return <<<'SQL'
...
SQL;
    ...
    private static function cartOrdersTableSql(): string
    {
        return <<<'SQL'
...
SQL;
    ...
    private static function cartDetailsTableSql(): string
    {
        return <<<'SQL'
...
SQL;
    ...
    private static function ensureCustomerGroupsTable(\PDO $pdo): void
    {
        $pdo->exec(self::customerGroupsTableSql());
    }

    private static function customerGroupsTableSql(): string
    {
        return <<<'SQL'
...
SQL;
    ...
    private static function catalogCategoriesTableSql(): string
    {
        return <<<'SQL'
...
SQL;
    ...
    private static function catalogProductsTableSql(): string
    {
        return <<<'SQL'
...
SQL;
    ...
    private static function ordersTableSql(): string
    {
        return <<<'SQL'
...
SQL;
    ...
    private static function orderDetailsTableSql(): string
    {
        return <<<'SQL'
...
SQL;
    ...
    {
        $exists = $pdo->query("SHOW TABLES LIKE 'users'");
        if ($exists === false || $exists->rowCount() === 0) {
            $pdo->exec(self::usersTableSql());
        }
    }

    private static function ensureSellerCatalogUserStateTable(\PDO $pdo): void
    {
        $pdo->exec(self::sellerCatalogUserStateTableSql());
        self::migrateSellerCatalogUserStateColumns($pdo);
    }

    private static function migrateSellerCatalogUserStateColumns(\PDO $pdo): void
    {
        $db = trim((string) config('DB_NAME', ''));
        if ($db === '') {
            return;
        ...
        $st->execute([$db, 'seller_catalog_user_state']);
        $have = [];
        foreach ($st->fetchAll(PDO::FETCH_COLUMN) as $col) {
            $have[strtolower((string) $col)] = true;
        }
        if (!isset($have['order_notes'])) {
            $pdo->exec(
                'ALTER TABLE `seller_catalog_user_state` ADD COLUMN `order_notes` TEXT NULL DEFAULT NULL AFTER `carts_json`'
            );
        }
    }

    private static function sellerCatalogUserStateTableSql(): string
    {
        return <<<'SQL'
...
SQL;
    ...
    private static function ensureCustomerCatalogUserStateTable(\PDO $pdo): void
    {
        $pdo->exec(self::customerCatalogUserStateTableSql());
    }

    private static function customerCatalogUserStateTableSql(): string
    {
        return <<<'SQL'
...
SQL;
    ...
    private static function ensureUsersListTable(\PDO $pdo): void
    {
        $pdo->exec(self::usersListTableSql());
        self::migrateUsersListColumns($pdo);
    }

    private static function migrateUsersListColumns(\PDO $pdo): void
    {
        $db = trim((string) config('DB_NAME', ''));
        if ($db === '') {
            return;
        ...
        $st->execute([$db, 'usersList']);
        $have = [];
        foreach ($st->fetchAll(PDO::FETCH_COLUMN) as $col) {
            $have[strtolower((string) $col)] = true;
        }
        if (!isset($have['json_payload'])) {
            $pdo->exec(
                'ALTER TABLE `usersList` ADD COLUMN `json_payload` LONGTEXT NULL AFTER `email_on_salesman_order`'
            );
        }
    }
    ...
    {
        $st = $pdo->prepare('SELECT userId FROM users WHERE username = ? LIMIT 1');
        $st->execute([self::ADMIN_USERNAME]);
        if ($st->fetch() !== false) {
            return;
        }
        ...
        $ins->execute([self::ADMIN_USERNAME, $hash, 1, 0]);
    }

    private static function customersRequestTableSql(): string
    {
        return <<<'SQL'
...
SQL;
    ...
    private static function customersTableSql(): string
    {
        return <<<'SQL'
CREATE TABLE `customers` (
  `customer_id` BIGINT NOT NULL AUTO_INCREMENT,
  `customeridfullvendor` BIGINT,
  `language_id` INT DEFAULT 1,
  `company_id` INT NOT NULL,
  `user_id` VARCHAR(255) DEFAULT ' ',
  `name` VARCHAR(255) DEFAULT ' ',
...
SQL;
    ...
    private static function usersTableSql(): string
    {
        return <<<'SQL'
CREATE TABLE `users` (
...
SQL;
    ...
    private static function usersListTableSql(): string
    {
        return <<<'SQL'
...
SQL;
    ...
    private static function ensureMessagingTables(\PDO $pdo): void
    {
        $pdo->exec(self::messagingConversationTableSql());
        $pdo->exec(self::messagingMessageTableSql());
        $pdo->exec(self::messagingReadTableSql());
    }

    private static function messagingConversationTableSql(): string
    {
        return <<<'SQL'
...
SQL;
    ...
    private static function messagingMessageTableSql(): string
    {
        return <<<'SQL'
...
SQL;
    ...
    private static function messagingReadTableSql(): string
    {
        return <<<'SQL'
